...
import { useRelationRecords } from "../../hooks/useRelationRecords.js";
import { useRelationConfig } from "../../providers/RelationConfigProvider.js";
import { RelationType } from "../../types/index.js";
import { RelationRecordsTable as ManyToManyRelationRecordsTable } from "../many-to-many/RelationRecordsTable.js";
import { RelationResourceActions as ManyToManyRelationResourceActions } from "../many-to-many/RelationResourceActions.js";
import { RelationRecordsTable as OneToManyRelationRecordsTable } from "../one-to-many/RelationRecordsTable.js";
import { RelationResourceActions as OneToManyRelationResourceActions } from "../one-to-many/RelationResourceActions.js";

export const RelationTab = () => {
  const { relation, ownerRecord, ownerResource, relations } =
    useRelationConfig();
  ...
  const junctionResourceId = relations[relation].junction?.throughResourceId;
  ...
  if (relationType === RelationType.ManyToMany) {
    ...
    const junctionResource = resources.find(
      (resource) => resource.id === junctionResourceId
    );
    ...
    return (
      <Box py="xl">
        ...
        />
        <ManyToManyRelationRecordsTable
          targetResource={targetResource}
          records={records}
          isLoading={isLoading}
        />
