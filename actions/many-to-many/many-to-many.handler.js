...
export const manyToManyHandler = async (
    ...
    const targetIds = junctionRecords.map(record => 
        record.params[junction.inverseJoinKey]
    );
    
    // Filter out undefined/null IDs
    const validTargetIds = targetIds.filter(id => 
    ...
    const orderedRecords = targetIds
        .map(id => targetRecords.find(record => record.id() === id))
        .filter(Boolean);

    // Populate related data
    const populatedRecords = await populator(orderedRecords, context);
